import type { DataLayerEvent, ProductParams, ProductFilter } from "../../types";
import { cleanValue, formatProduct } from "../utils/cleanValue"

export default function productListingModule(datalayer: any) {
	return {
		 ...
		 * );
		 */
		view: (productsArray: ProductParams[], listName?: string): DataLayerEvent => {
			...
			return datalayer.pushEvent("product_listing-view", {
				default: {
					page: {
						type: "product",
						action: "listing-view",
		...
		filter: (filterData: ProductFilter): DataLayerEvent => {
			return datalayer.pushEvent("product_listing-filters", {
				default: {
					page: {
						type: "product",
						action: "listing-view",
		...
		sort: (option: string): DataLayerEvent => {
			return datalayer.pushEvent("product_listing-sort", {
				default: {
					page: {
						type: "product",
						action: "listing-sort",
